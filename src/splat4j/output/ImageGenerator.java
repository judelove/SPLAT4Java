...
public class ImageGenerator {
    ...
    public ImageGenerator(Configuration config, SplatEngine splat) {
        this.config = config;
        this.splat = splat;
    }

    public void WriteImage(String baseFilename, boolean geo, boolean kml, boolean ngs, Site[] xmtr) {
        ...
        try {
            ...
            File mapFile = new File(baseFilename + (splat.isGeneratePpm() ? ".ppm" : ".png"));
            System.out.printf("\nWriting \"%s\" (%sx%s pixmap image)... ", mapFile, (int) width, (int) height);
                        ...
                        for (x = 0, lon = splat.getMaxWest(); x < (int) width; x++, lon = (double) splat.getMaxWest() - (splat.getDpp() * (double) x)) {
                            ...
                            if (found) {
                                ...
                                if ((mask & 2) > 0) /* Text Labels: Red */ {
                                    wri.write(new byte[]{(byte) 255, (byte) 0, (byte) 0});
                                    ...
                                    switch (mask & 57) {
                                        ...
                                        default:
                                            ...
                                            } else /* Sea-level: Medium Blue */ if (splat.getDem()[indx].getData(x0, y0) == 0) {
 ...
 /* we do, display the region as black */
                ...
                BufferedImage img = new BufferedImage((int) width, (int) height, splat.isTransparentPng() ? BufferedImage.TYPE_INT_ARGB : BufferedImage.TYPE_INT_RGB);
                for (y = 0, lat = north; y < (int) height; y++, lat = north - (splat.getDpp() * (double) y)) {
                    for (x = 0, lon = splat.getMaxWest(); x < (int) width; x++, lon = (double) splat.getMaxWest() - (splat.getDpp() * (double) x)) {
                        if (lon < 0.0) {
                            lon += 360.0;
                        ...
                        if (found) {
                            ...
                            if ((mask & 2) > 0) /* Text Labels: Red */ {
                                ...
                                switch (mask & 57) {
                                    ...
                                    default:
                                        if (ngs) /* No terrain */ {
                                            img.setRGB(x, y, (splat.isTransparentPng() ? 0x00000000 : 0xFF000000) + (255 << 16) + (255 << 8) + 255);
                                        } else /* Sea-level: Medium Blue */ if (splat.getDem()[indx].getData(x0, y0) == 0) {
                                            img.setRGB(x, y, 0xFF000000 + (0 << 16) + (0 << 8) + 170);
                                        } else {
                                            /* Elevation: Greyscale */
                                            terrain = (0.5 + Math.pow((double) (splat.getDem()[indx].getData(x0, y0) - splat.getMinElevation()), one_over_gamma) * conversion);
                                            img.setRGB(x, y, (splat.isTransparentPng() ? 0x00000000 : 0xFF000000) + ((int) terrain << 16) + ((int) terrain << 8) + (int) terrain);
                                        }
                                }
                            }
                        } else {
 ...
 /* we do, display the region as black */
    ...
    public void WriteLRImage(String baseFilename, boolean geo, boolean kml, boolean ngs, Site[] xmtr, Dem[] dem) {
                        ...
                        if (found) {
                            ...
                            if (!cityorcounty) {
                                if (loss == 0 || (splat.getContourThreshold() != 0 && loss > Math.abs(splat.getContourThreshold()))) {
                                    ...
                                    } else /* Display land or sea elevation */ if (dem[indx].getData()[x0][y0] == 0) {
                                ...
                                } else /* Plot path loss in color */ if (red != 0 || green != 0 || blue != 0) {
                                    mpWri.write(new byte[]{(byte) red, (byte) green, (byte) blue});
                                } else /* terrain / sea-level */ if (dem[indx].getData()[x0][y0] == 0) {
                                    mpWri.write(new byte[]{(byte) 0, (byte) 0, (byte) 170});
                                } else {
                                    /* Elevation: Greyscale */
                                    terrain = (int) (0.5 + Math.pow((double) (dem[indx].getData()[x0][y0] - splat.getMinElevation()), one_over_gamma) * conversion);
 ...
 /* we do, display the region as black */
                ...
                * if not generating .kml or .geo output.
                ...
                Logger.getLogger(ImageGenerator.class
            ...
            BufferedImage img = new BufferedImage((int) width, (int) height, splat.isTransparentPng() ? BufferedImage.TYPE_INT_ARGB : BufferedImage.TYPE_INT_RGB);
            int prefix = splat.isTransparentPng() ? 0x00000000 : 0xFF000000;
            for (y = 0, lat = north; y < (int) height; y++, lat = north - (splat.getDpp() * (double) y)) {
                for (x = 0, lon = splat.getMaxWest(); x < (int) width; x++, lon = (double) splat.getMaxWest() - (splat.getDpp() * (double) x)) {
                    if (lon < 0.0) {
                        lon += 360.0;
                    ...
                    if (found) {
                        ...
                        if (!cityorcounty) {
                            if (loss == 0 || (splat.getContourThreshold() != 0 && loss > Math.abs(splat.getContourThreshold()))) {
                                if (ngs) /* No terrain */ {
                                    img.setRGB(x, y, prefix + (255 << 16) + (255 << 8) + 255);
                                } else /* Display land or sea elevation */ if (dem[indx].getData()[x0][y0] == 0) {
                                    img.setRGB(x, y, 0xFF000000 + (0 << 16) + (0 << 8) + 170);
                                } else {
                                    terrain = (int) (0.5 + Math.pow((double) (dem[indx].getData()[x0][y0] - splat.getMinElevation()), one_over_gamma) * conversion);
                                    img.setRGB(x, y, prefix + ((int) terrain << 16) + ((int) terrain << 8) + (int) terrain);
                                }
                            } else /* Plot path loss in color */ if (red != 0 || green != 0 || blue != 0) {
        ...
        if (kml) {
            ...
            try {
                ...
                BufferedImage ckBf = null;
                File ckFile = new File(baseFilename + (splat.isGeneratePpm() ? "-ck.ppm" : "-ck.png"));
                ...
                if (splat.isGeneratePpm()) {
                    ...
                    ckBf = new BufferedImage((int) width, (int) height, BufferedImage.TYPE_INT_ARGB);
                }
                    ...
                    for (x0 = 0; x0 < (int) width; x0++) {
                        ...
                        if (indx > region.getLevels()) {
                            if (splat.isGeneratePpm()) {
                                ...
                                ckBf.setRGB(x, y, 0xFF000000 + (0 << 16) + (0 << 8) + 0);
                            }
                            ...
                            if (splat.isGeneratePpm()) {
                                ...
                                ckBf.setRGB(x, y, 0xFF000000 + (red << 16) + (green << 8) + blue);
                            }
                        }
                    }
                }
                if (!splat.isGeneratePpm()) {
                    ImageIO.write(ckBf, "png", ckFile);
                }
    ...
    private void notifyImageFileWrite(FileOutputStream mpWri, String baseFilename) throws IOException {
    ...
    private void writeGeoFile(String baseFilename, double west, double north, double width, double height, double east, double south) {
        ...
        try {
            ...
            wri.write(String.format("FILENAME\t%s\n", baseFilename + (splat.isGeneratePpm() ? ".ppm" : ".png")));
    ...
    private void writeKMLFile(String baseFilename, Site[] xmtr, double north, double south, double east, double west, int x) {
        ...
        try {
            ...
            wri.write("         <name>SPLAT4J! Path Loss Overlay</name>\n");
            wri.write("           <description>FM Signal Coverage</description>\n");
            wri.write("		<Icon>\n");
            ...
            wri.write("          <Icon>\n");
            wri.write(String.format("            <href>%s</href>\n", baseFilename + (splat.isGeneratePpm() ? "-ck.ppm" : "-ck.png")));
            wri.write("          </Icon>\n");
            wri.write("          <overlayXY x=\"10\" y=\"10\" xunits=\"fraction\" yunits=\"fraction\"/>\n");
    ...
    public void WriteSSImage(String baseFilename, boolean geo, boolean kml, boolean ngs, Site[] xmtr, Dem[] dem) {
        Region region = new Region();
                    ...
                    for (x = 0, lon = splat.getMaxWest(); x < (int) width; x++, lon = (double) splat.getMaxWest() - (splat.getDpp() * (double) x)) {
                        ...
                        if (found) {
                            ...
                            if (!cityorcounty) {
                                ...
                                } else /* Plot field strength regions in color */ if (red != 0 || green != 0 || blue != 0) {
                                    mpWri.write(new byte[]{(byte) red, (byte) green, (byte) blue});
                                } else /* terrain / sea-level */ if (ngs) {
                                    mpWri.write(new byte[]{(byte) 255, (byte) 255, (byte) 255});
                                } else if (dem[indx].getData(x0, y0) == 0) {
                                    mpWri.write(new byte[]{(byte) 0, (byte) 0, (byte) 170});
                                } else {
                                    /* Elevation: Greyscale */
                                    terrain = (0.5 + Math.pow((double) (dem[indx].getData(x0, y0) - splat.getMinElevation()), one_over_gamma) * conversion);
 ...
 /* we do, display the region as black */
                    ...
                    for (x0 = 0; x0 < (int) width; x0++) {
                        ...
                        if (indx > region.getLevels()) {
                            if (splat.isGeneratePpm()) {
                                ...
                                ckBf.setRGB(x0, y0, 0xFF000000 + (0 << 16) + (0 << 8) + 0);
                            }
                            ...
                            if (splat.isGeneratePpm()) {
                                ...
                                ckBf.setRGB(x0, y0, 0xFF000000 + (red << 16) + (green << 8) + blue);
                            }
                        }
    ...
    public void WriteDBMImage(String baseFilename, boolean geo, boolean kml, boolean ngs, Site[] xmtr) {
        Region region = new Region();
                        ...
                        if (found) {
                            ...
                            if (!cityorcounty) {
                                if (splat.getContourThreshold() != 0 && dBm < splat.getContourThreshold()) {
                                    if (ngs) /* No terrain */ {
                                        mpWri.write(new byte[]{(byte) 255, (byte) 255, (byte) 255});
                                    } else /* Display land or sea elevation */ if (splat.getDem()[indx].getData(x0, y0) == 0) {
                                        mpWri.write(new byte[]{(byte) 0, (byte) 0, (byte) 170});
                                    } else {
                                        terrain = (0.5 + Math.pow((double) (splat.getDem()[indx].getData(x0, y0) - splat.getMinElevation()), one_over_gamma) * conversion);
                                        mpWri.write(new byte[]{(byte) terrain, (byte) terrain, (byte) terrain});
                                    }
                                } else /* Plot signal power level regions in color */ if (red != 0 || green != 0 || blue != 0) {
                                    mpWri.write(new byte[]{(byte) red, (byte) green, (byte) blue});
                                } else /* terrain / sea-level */ if (ngs) {
                                    mpWri.write(new byte[]{(byte) 255, (byte) 255, (byte) 255});
                                } else if (splat.getDem()[indx].getData(x0, y0) == 0) {
                                    mpWri.write(new byte[]{(byte) 0, (byte) 0, (byte) 170});
                                } else {
                                    /* Elevation: Greyscale */
                                    terrain = (0.5 + Math.pow((double) (splat.getDem()[indx].getData(x0, y0) - splat.getMinElevation()), one_over_gamma) * conversion);
                    ...
                    if (found) {
                        ...
                        if (!cityorcounty) {
                            if (splat.getContourThreshold() != 0 && dBm < splat.getContourThreshold()) {
                                ...
                                img.setRGB(x, y, 0xFF000000 + (0 << red) + (0 << green) + blue);
                            } else /* terrain / sea-level */ if (ngs) {
                                img.setRGB(x, y, 0xFF000000 + (255 << 16) + (255 << 8) + 255);
                            } else if (splat.getDem()[indx].getData(x0, y0) == 0) {
                                img.setRGB(x, y, 0xFF000000 + (0 << 16) + (0 << 8) + 170);
